...
class MarketPlotter:
    ...
    def plot_portfolio_value(self):
        ...
        plt.plot(self.df["tick"], self.df["realized_pl"], label="Realized P/L", alpha=0.7)
        plt.plot(self.df["tick"], -self.df["total_fees"], label="Total Fees Paid (negative)", alpha=0.5, linestyle="--")
        plt.title("Profit and Fees Over Time")
        plt.xlabel("Tick")
        plt.ylabel("Amount ($)")
        plt.legend()
        plt.grid(True)
